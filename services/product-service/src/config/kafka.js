...
const connectKafka = async () => {
    ...
    while (retries > 0) {
        try {
            ...
            console.log('✅ [Product] Kafka Producer & Consumer connected successfully');
            return;
        } catch (error) {
            console.error(`❌ [Product] Failed to connect to Kafka. Retries left: ${retries - 1}. Error: ${error.message}`);
            retries -= 1;
            if (retries === 0) {
