// Script: Ghi root package.json + fix tat ca require paths
const fs = require('fs');
const path = require('path');

const ROOT = path.join(__dirname, '..');

// ========== 1. Viet root package.json ==========
const rootPkg = {
  name: 'shopee-microservices',
  version: '2.0.0',
  description: 'Monorepo single package.json',
  scripts: {
    ...
    postinstall: 'node services/_fix-paths.js'
  },
  dependencies: {
    argon2: '^0.41.1',
    axios: '^1.14.0',
...
// ========== 2. Remove sub package.jsons & node_modules ==========
...
services.forEach(svc => {
  ...
  const nodeModules = path.join(svcDir, 'node_modules');
  
  if (fs.existsSync(pkgFile)) {
    fs.renameSync(pkgFile, pkgFile + '.backup');
    console.log('  📦 Backed up ' + svc + '/package.json → .backup');
  }
  if (fs.existsSync(nodeModules)) {
    fs.rmSync(nodeModules, { recursive: true, force: true });
    console.log('  🗑️  Removed ' + svc + '/node_modules');
  }
});
